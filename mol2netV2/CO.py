...
#####################PREAMBLE
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
trafos          =       ['H2', 'O','NH3'] #modified e-mail/why? #['H2','O','CO','NH3','CHN','H2O','CO2','CH2O','CHNO']
...
plt.savefig('NetworkRepresentation_clean_CO'+'.png')
...
for color_type in l:
    ...
    plt.axis('off')
    plt.savefig('fCSoff_Supporting Figure 7 - ' + color_type + '_minimal_set_CO_all_components' + '.pdf')
    plt.show()
...
plt.axis('off')
plt.savefig('fCSoff_Sum_of_Standard_Deviations_CO.pdf')
plt.show()
...
plt.axis('off')
plt.savefig('Soff_Top_5_Subgraphs_with_Smallest_Std_Color_CO.pdf')
plt.show()
...
shades_of_orange = ['#0000FF', '#4169E1', '#6495ED', '#4682B4', '#87CEEB'] #actually blue now
...
plt.axis('off')
plt.savefig('fCSoff_Top_5_Subgraphs_with_Smallest_Std_CO.pdf')
plt.show()
fCxNODES_annotated2 = xNODES.copy()
#Saving the dataframes so you don't have to compile everything again if you want to change something 

fCxNODES_annotated2.to_csv("fCxNODES_annotated2.tsv")
xNODES.to_csv("Nodes_CO_Soff.tsv")
